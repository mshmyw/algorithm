...
typedef struct UF{
    ...
    int id_length;
    int count;
    int *sz; // because of trigger point, got the sub size
    int sz_length;
}UF, *PUF;
...
void quick_union(PUF puf, int p, int q){
   int p_root = find(puf, p);
    ...
    if(puf->sz[p_root] < puf->sz[q_root]){
        puf->id[p_root] = q_root;
        puf->sz[q_root] += puf->sz[p_root];
    }else{
        puf->id[q_root] = p_root;
        puf->sz[p_root] += puf->sz[q_root];
    }
    puf->count--;
